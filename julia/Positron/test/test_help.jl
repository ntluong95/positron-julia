...
@testset "Help Service Tests" begin
    ...
    @testset "HelpShowHelpParams Serialization" begin
        # HTML kind
        params = Positron.HelpShowHelpParams(
            "<html>content</html>",
            Positron.ShowHelpKind_Html,
            true,
        )
        ...
        # Markdown kind
        params =
            Positron.HelpShowHelpParams("# Markdown", Positron.ShowHelpKind_Markdown, false)
        json_str = JSON3.write(params)
        parsed = JSON3.read(json_str, Dict)
    ...
    @testset "HelpService - Handle Help Topic" begin
        ...
        events = filter(
            m -> haskey(m, "data") && m["data"] isa Positron.JsonRpcNotification,
            comm.messages,
        )
